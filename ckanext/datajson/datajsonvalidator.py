...
omb_file = urlopen("https://project-open-data.cio.gov/data/omb_bureau_codes.csv")
omb_csv = omb_file.read().decode('utf-8')
for row in csv.DictReader(omb_file.read().decode('utf-8')):
    ...
    if type(doc) != list:
        ...
        for i, doc_item in enumerate(doc):
            ...
            for item in doc_item.get('dataset', []):
                ...
                # keyword # required
                if isinstance(item.get("keyword"), (str, str)):
                ...
                if item.get("distribution") is None:
                    ...
                    if isinstance(item["distribution"], (str, str)) and is_redacted(item.get("distribution")):
                        pass
                    else:
                        add_error(errs, 50, "Invalid Field Value (Optional Fields)",
                ...
                if item.get("spatial") is not None and not isinstance(item.get("spatial"), (str, str)):
                    add_error(errs, 50, "Invalid Field Value (Optional Fields)",
                ...
                if item.get("temporal") is None or is_redacted(item.get("temporal")):
                    pass  # not required or REDACTED
                elif not isinstance(item["temporal"], (str, str)):
                ...
                if item.get("references") is None:
                    ...
                    if isinstance(item["references"], (str, str)) and is_redacted(item.get("references")):
                        pass
                    else:
                        add_error(errs, 50, "Invalid Field Value (Optional Fields)",
...
def is_redacted(field):
    if isinstance(field, (str, str)) and REDACTED_REGEX.match(field):
        return True
    return False
